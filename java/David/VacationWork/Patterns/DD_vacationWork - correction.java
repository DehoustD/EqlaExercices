...
// Super ! Je t'ai mis quelques petites notes de correction, ou plutôt d'amélioration, mais ce dont des broutilles. Excellent travail, continue, tu vas tout casser en prog ;)
public class DD_vacationWork {
    ...
    static boolean AskYN(String _question) {
        ...
        // Cor - On va plutôt utiliser le terme "answer" ou "userAnswer" habituellement, aussi ta fonction pourrait s'appeler entièrement AskYesOrNo() simplement pour le confort de lecture de tes éventuels lecteurs/gens qui travaille avec toi. C'est plus instinctif de lire l'appel qui dit looping = AskYesOrNo("\nVoulez vous recommencer ? (y/n) "); que en abrégé. Mais ce n'est pas mauvais pour autant bien sur !
    ...
    static void ShowPattern(int _index, int _max) {
        ...
        // Cor - Pour tes noms de fonction, n'hésite pas à pousser le vice de les traduire en anglais, ça ne me dérange pas que l'on code en français, mais le franglais c'est un dommage. Par exemple "CreateInvertedTriangle" ;) c'est du détail hein, mais bon !
        switch (_index) {
            case 1:
